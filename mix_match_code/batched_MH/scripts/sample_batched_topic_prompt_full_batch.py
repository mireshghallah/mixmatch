...
def get_agency_dict(agency_file):
    ...
    agency_dict = pd.read_csv(agency_file, header=None, index_col=0, squeeze=True).to_dict()
    ...
    for i, key in enumerate(agency_dict.keys()):
        ...
        if i == 0:
            continue 
        agency_dict_main[key.strip()]=agency_dict[key]
    
    ag_dict = {'0':[],'1':[], '2':[],'3':[],'4':[],'5':[],'6':[]}
    dict_src = {'computers':'0', 'legal':'1', 'military':'2', 'politics':'3','religion':'4', 'science':'5', 'space':'6'}
    
    for key in agency_dict_main.keys():
        if(agency_dict_main[key] == agency_dict_main[key]):
            ag_dict[dict_src[agency_dict_main[key.strip()]]].append(key.strip())
...
def get_opt_sent(sents,metadata):
    ...
    return " ".join(sent_best), meta_data[ind][-4],ind
...
def get_init_text(seed_text, max_len, batch_size=1, rand_init=False):
    ...
    batch = [ [CLS]+ seed_text[i] + [MASK]*(max_len - len(seed_text[i])) + [SEP]  for i in range(batch_size)] #TODO
...
def parallel_sequential_generation(
    ...
    for ii in range(max_iter):
        ...
        for positions in groups:
            ...
            if args.degenerate: 
                ...
                disc_1,disc_2, disc_preds = 0,0,0 #energy_score_disc(batch,model_disc=model_disc,tokenizer_disc=tokenizer_disc,sentiment=sentiment,alpha=args.alpha)
               ...
               #print("new dist\n",distance)
                disc_1,disc_2, disc_preds_new = 0,0,0 #energy_score_disc(batch,model_disc=model_disc,tokenizer_disc=tokenizer_disc,sentiment=sentiment,alpha=args.alpha)
                ...
                r_score  = np.squeeze(np.where(acc>0.0,new_r,old_r) )
                norm_score =np.squeeze( np.where(acc>0.0,new_norm,old_norm) )
                disc_preds  = 0 # np.squeeze(np.where(acc>0.0,disc_preds_new,disc_preds) )   
                ...
                for i in range(batch_size):
                    full_meta_data[i].append( (sentiment, r_score[i],norm_score[i],qxxb[i],qxbx[i],axbx[i],acc[i].item(),disc_preds, distance[i] ,bleurt[i], lexicon[i]))
    ...
    for i in range(batch_size):
        meta_data.append( (sentiment, r_score[i],norm_score[i],qxxb[i],qxbx[i],axbx[i],acc[i].item(),disc_preds ,distance[i],bleurt[i]))
...
max_len = args.max_len #1 #this is dummy!!
########
...
if args.degenerate:
    folder_name = "mask_degenerate_max_len_{}_topk_{}_burnin_{}_disc_{}_data_{}_max_iter_{}_temp_{}_shuffle_{}_block_{}_alpha_{}_beta_{}_delta_{}_gamma_{}_theta_{}_date_{}".format(max_len,top_k,burnin, args.disc_name,args.data_name,max_iter,temperature,args.shuffle_positions,args.block,args.alpha,args.beta,args.delta, args.gamma, args.theta ,dt_string)    
else:
    folder_name = "mask_disc_max_len_{}_{}_data_{}_max_iter_{}_temp_{}_shuffle_{}_block_{}_alpha_{}_beta_{}_delta_{}_gamma_{}_theta_{}_date_{}".format(args.max_len,args.disc_name,args.data_name,max_iter,temperature,args.shuffle_positions,args.block, args.alpha,args.beta, args.delta,args.gamma, args.theta, dt_string)
...
seed_text_list =[]
with open(f"{dirname}/samples.txt", "w") as f, open(f"{dirname}/opt_samples.txt", "w") as optimal_f,open(f"{dirname}/opt_cls.txt", "w") as optimal_class,open(f"{dirname}/opt_meta.txt", "w") as opt_meta_file ,open(f"{dirname}/metadata.txt", "w") as f_meta ,  open(f"{data_dir_src}", "r") as src_file, open(f"{attr_dir}", "r") as attr_file:
    for i,(line, src) in enumerate(zip(src_file, attr_file)):
        
        seed_text = line[:-1]
        sentiment = int(int(src[:-1]))
        print(seed_text)
    ...
    sents = list(map(lambda x: " ".join(detokenize(x)[1:-1]), bert_sents))
