...
def get_opt_sent(sents,metadata):
    ...
    return " ".join(sent_best[1:-1]), meta_data[ind][-1],ind
...
def parallel_sequential_generation(
    ...
    for i in range(batch_size):
        meta_data.append( (sentiment, r_score[i],norm_score[i],qxxb[i],qxbx[i],axbx[i],acc[i].item(),disc_preds[i] ,distance[i],bleurt[i],lexicon[i]))
