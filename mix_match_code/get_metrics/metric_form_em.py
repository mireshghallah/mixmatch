...
def get_gpt_ppl(checkpoint_dir, file_name='opt_samples.txt',):
    ...
    with open(f"{checkpoint_dir}/{file_name}", "r") as input_file:
        for line in input_file:
            ...
            if len(sent_list_temp.split()) > 400 and len(sent_list_temp.split()) < 450: 
                ...
                sent_list = ""
            elif len(sent_list_temp.split())<400:
                sent_list = sent_list_temp
                continue
            else: # > 450
                encoded_input = gpt_tokenizer(text, padding=True, return_tensors='pt').to(device)
    ...
    with torch.no_grad():
        with open(f"{checkpoint_dir}/{file_name}", "r") as input_file:
                ...
                for line in input_file:
                    ...
                    sentence=sentence.replace(' )', ')')
                    sentence=sentence.replace('( ', '(')
                    sentence=sentence.replace('& apos ;','')
                    sentence=sentence.replace(' !','!')
                    sentence=sentence.replace(' ?','?')
                    sentence=sentence.replace('- |','')
...
gpt_mean_score = perplexity_fudge(checkpoint_dir) #get_gpt_ppl(checkpoint_dir,file_name='opt_samples.txt')

###Bertscore
self_bertsc = get_bert_score(txt_file,checkpoint_dir) 
bertsc = get_bert_score(ref_file+'0',checkpoint_dir) 
